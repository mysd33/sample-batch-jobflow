...
public class Job913Tasklet implements Tasklet {
    ...
    public RepeatStatus execute(@NonNull StepContribution contribution, @NonNull ChunkContext chunkContext) throws Exception {
        ...
        Job913InputData job913InputData = objectMapper.readValue(inputData, Job913InputData.class);

        // Job911の処理結果取得
        appLogger.debug("Job911から受け取った処理結果[result:{}]", job913InputData.getJobResultList().getFirst());

        // Job912の処理結果取得
        appLogger.debug("Job912から受け取った処理結果[result:{}]", job913InputData.getJobResultList().get(1));

        // 処理結果はダミーの値をセットしている。
        // 実際はJob913Taskletの処理結果をセットする。
        Job913ResultData job913ResultData = Job913ResultData.builder().result("result_job913").build();
